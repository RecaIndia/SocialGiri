        ...
        private async void BtnApplyOnClick(object sender, EventArgs e)
        {
            try
            {
                if (Methods.CheckConnectivity())
                {
                    ...
                    {
                        (apiStatus, respond) = await RequestsAsync.Global.UploadBankRecipeAsync(Id, Price, "Pay the card " + Price, PathImage);
                    }
                    else if(PayType == "AddFunds" || PayType == "SendMoney")
                    {
                        //wallet
                        (apiStatus, respond) = await RequestsAsync.Global.UploadBankRecipeAsync("wallet", Price, "Pay the card " + Price, PathImage);
                    }
