...
namespace WoWonder.Activities.AddPost.Service
{
    ...
    public class ReceivingSharingActivity : AppCompatActivity, SlidingPaneLayout.IPanelSlideListener, SlidingUpPanelLayout.IPanelSlideListener, MaterialDialog.IListCallback, MaterialDialog.ISingleButtonCallback, MaterialDialog.IInputCallback
    {
        ...
        private Android.Net.Uri UriData;
        ...
        protected override void OnActivityResult(int requestCode, Result resultCode, Intent data)
        {
            try
            {
                ...
                else if (requestCode == 3 && resultCode == Result.Ok) // Mention
                {
                    try
                    {
                        var user = MentionActivity.MAdapter.MentionList.Where(a => a.Selected).ToList();
                        if (user.Count > 0)
                        {
                            foreach (var item in user) MentionText += " @" + item.Username + " ,";
