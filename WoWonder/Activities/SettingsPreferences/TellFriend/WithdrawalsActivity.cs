...
namespace WoWonder.Activities.SettingsPreferences.TellFriend
{
    ...
    public class WithdrawalsActivity : AppCompatActivity, MaterialDialog.IListCallback, MaterialDialog.ISingleButtonCallback
    {
        ...
        private async void BtnRequestWithdrawalOnClick(object sender, EventArgs e)
        {
            try
            {
                ...
                if (TypeWithdrawMethod == "paypal" && string.IsNullOrEmpty(TxtPayPalEmail.Text.Replace(" ", "")) || string.IsNullOrEmpty(TxtAmount.Text.Replace(" ", "")))
                {
                    Toast.MakeText(this, GetText(Resource.String.Lbl_Please_check_your_details), ToastLength.Long).Show();
                    return;
                }
                 
                if (TypeWithdrawMethod == "bank" && string.IsNullOrEmpty(TxtAmount.Text.Replace(" ", "")) || string.IsNullOrEmpty(TxtAccountNumber.Text.Replace(" ", "")) || string.IsNullOrEmpty(TxtCountry.Text.Replace(" ", ""))
